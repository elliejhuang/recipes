...
export function RecipeCard({
  ...
  /** Edit/delete, shown over the photo — only where a list is the sole way
   *  to reach a recipe without opening it first. */
  showActions?: boolean;
}) {
  ...
  return (
    <Link href={`/recipes/${recipe.id}`} className="group relative block">
      ...
      {showActions && <RecipeCardActions id={recipe.id} title={recipe.title} />}
...
export function RecipeGrid({
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
      {lead}
      {recipes.map((recipe) => (
        <RecipeCard key={recipe.id} recipe={recipe} showActions={showActions} />
      ))}
    </div>
  );
}
