      ...
      <body className="min-h-dvh">
        <header className="no-print sticky top-0 z-30 border-b border-rule bg-paper/85 backdrop-blur">
          <div className="mx-auto flex h-14 max-w-5xl items-center gap-1 px-4">
            <Link
              href="/"
              className="mr-3 font-serif text-lg font-semibold tracking-tight"
            >
              Recipe Box
            </Link>
            ...
            <nav className="flex items-center gap-1">
              {NAV.map(({ href, label, icon: Icon }) => (
                <Link
                  key={href}
                  href={href}
                  className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm text-muted hover:bg-card hover:text-ink"
                >
